...
with tab3:
    ...
    ax4.text(-0.05, egg_stage_duration + nymphal_stage_duration/2, 'NYMPHS', rotation=90, 
             transform=ax4.get_yaxis_transform(), va='center', ha='right', fontweight='bold')
    ax4.text(-0.05, egg_stage_duration + nymphal_stage_duration + 5, 'ADULTS', rotation=90, 
             transform=ax4.get_yaxis_transform(), va='center', ha='right', fontweight='bold')
...
st.header("About the Leslie Matrix Model for Cockroaches")
st.markdown("""
The Leslie matrix model is a powerful tool for modeling age-structured populations, particularly useful for species like cockroaches that have distinct life stages and age-specific reproduction.

### Cockroach Life Cycle:
1. **Egg Stage**: Cockroach eggs are contained in oothecae (egg cases), which protect them during development
2. **Nymphal Stage**: Immature cockroaches that undergo multiple molts before reaching adulthood
3. **Adult Stage**: Sexually mature cockroaches that can reproduce

### Model Components:
- **Stage-specific survival rates**: Different survival probabilities for eggs, nymphs, and adults
- **Age-specific fecundity**: Female cockroaches produce oothecae at specific points in their adult life
- **Initial population**: Starting population in a particular age class

### Applications:
- Pest management planning and evaluation
- Understanding cockroach population dynamics in different environments
- Testing the effectiveness of different control strategies
- Predicting infestation potential and spread rates

This model can be particularly valuable for:
- Integrated Pest Management (IPM) programs
- Public health initiatives targeting disease vectors
- Research on insect population ecology
- Urban entomology studies

By adjusting parameters like survival rates and fecundity, you can simulate different environmental conditions or control measures to understand their impact on cockroach populations.
""")
